...
def extract_symptoms_with_matcher(
    ...
    Returns:
      {
        "labels": [..],   # symptom labels (strings) for rdf_disease_finder
        "iris":   [..],   # full IRIs (strings) for triage_engine_queries
        "matches": [..],  # raw match dicts
      }
    """
...
def run_diagnosis(text: str, components: Dict[str, Any]):
    ...
    extracted = extract_symptoms_with_matcher(
        text=text,
    ...
    symptoms = extracted["labels"]         # labels voor rdf_disease_finder
    symptom_iris = extracted["iris"]       # IRIs voor triage
    ...
    try:
        # Load KG graph for triage (Windows-safe parse)
        ttl_path = (base_dir / "ontology" / "databaseV7.ttl").resolve()
        g = Graph()
        g.parse(ttl_path.as_uri(), format="turtle")

        disease_iri = None
        # Prefer disease_uri from KG candidate if available
        if nearest_diseases:
            disease_iri = nearest_diseases[0].get("disease_uri")

        triage_result = triage_case(
            g=g,
            user_text=text,
            symptom_iris=symptom_iris,   # <-- full IRIs from matcher
