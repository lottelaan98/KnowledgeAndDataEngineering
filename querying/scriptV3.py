...
# ============================================================
# Optional: keep a demo runner, but only when executed directly
# ============================================================
if __name__ == "__main__":
    ttl = r"C:\Users\lotte\Documents\GitHub\KnowledgeAndDataEngineering\ontology\databaseV7.ttl"
    g = safe_load_graph(ttl)
    if g is None:
        raise SystemExit(1)

    print("\n--- DEMO: Query 1 ---")
    symps_q1 = ["wd:Q9690", "wd:Q38933"]
    # , "wd:Q81938", "wd:Q186889", "wd:Q127076"
    exc = "Typhoid"
    rows = query1_topk_diseases_by_score(g, symps_q1, top_k_results=3, exclude_label=exc)
    for i, r in enumerate(rows, 1):
        print(f"{i}. Disease: {r['label']}   Score: {r['finalScore']:.3f}")
        print(f"   URI: {r['disease_uri']}")

    # Pick a disease for demo
    disease_iri = "http://www.wikidata.org/entity/Q83319"

    print("\n--- DEMO: Query 2 ---")
    info = query2_disease_wiki_info(g, disease_iri)
    if info:
        print(f"Disease: {info['label']}\nExplanation: {info['explanation']}\nVisit to the doctor: {info['seeDoctor']}\nTreatment: {info['treatment']}")
    else:
        print("(no info found)")

    print("\n--- DEMO: Query 3 ---")
    symps_q3 = ["wd:Q2536390", "wd:Q29644032", "wd:Q38933", "wd:Q9690", "wd:Q86", "wd:Q87"]
